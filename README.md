...
- Composeとイメージは `TZ=Asia/Tokyo` を設定し、Goにタイムゾーンデータを組み込みます。サーバーログは日本時間です。
- 画面の記録時刻と対局日の初期値は、端末の設定に関係なく日本時間です。
- 新規記録・インポート時の記録日時とバックアップ出力日時はUTCを使用します。既存日時はオフセットを含めて解釈し、実際の時刻順に表示します。
- DB・API・バックアップの形式は維持し、既存データの移行は行いません。
